...
function! LatexEvinceSearch()
    silent execute '!cd "%:h"; ' . s:sp . 'evince_vim_dbus.py EVINCE "%:r.pdf" ' . line('.') . ' "%:p"'
endfun

" Map forward search to ls
let s:sp = expand('<sfile>:h') . '/../scripts/'
command! LatexEvinceSearch call LatexEvinceSearch()
nmap <silent> ls :LatexEvinceSearch<CR>


" Below follows a modified version of tex_pdf.vim, augmented with
" Evince reverse search support. I'll tag my changes with XXX.
"
